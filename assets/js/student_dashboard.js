...
// Backwards-compatible wrapper used by inline buttons in the PHP dashboard
function viewMaterials(courseId, courseCode) {
    ...
    fetch(`../PHP/get_course_materials.php?course_id=${encodeURIComponent(courseId)}`)
        .then(res => res.json())
        .then(data => {
            if (!data.success) {
                content.innerHTML = `<div class="alert alert-danger">${data.message || 'Error loading materials'}</div>`;
                return;
            }
                    ...
                    <div class="list-group-item">
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1">
                                <h6 class="mb-1"><i class="${fileIcon} me-2"></i>${material.title}</h6>
                                <p class="mb-1 text-muted">${material.description || 'No description available'}</p>
                                <small class="text-muted">Uploaded by ${material.lecturer_name || 'Lecturer'} on ${createdDate}</small>
                            </div>
                            <div class="ms-3">
                                <a href="${material.file_path_url}" class="btn btn-primary btn-sm" target="_blank"><i class="fas fa-download me-1"></i>Download</a>
                            </div>
                        </div>
                    </div>
                `;
...
function viewAssignments(courseId, courseCode) {
    ...
    fetch(`../PHP/get_course_assignments.php?course_id=${encodeURIComponent(courseId)}`)
        .then(res => res.json())
        .then(data => {
            if (!data.success) {
                content.innerHTML = `<div class="alert alert-danger">${data.message || 'Error loading assignments'}</div>`;
                return;
            }
                ...
                html += `
                    <div class="list-group-item">
                        <div class="d-flex justify-content-between align-items-start">
                            ...
                            <div class="ms-3">
                                ${!isSubmitted && !isOverdue ? 
                                    `<button class="btn btn-primary btn-sm" onclick="submitAssignment(${assignment.assignment_id})"><i class="fas fa-upload me-1"></i>Submit</button>` : 
                                    `<button class="btn btn-outline-secondary btn-sm" disabled><i class="fas fa-eye me-1"></i>View</button>`
                                }
                            </div>
                        </div>
                    </div>
