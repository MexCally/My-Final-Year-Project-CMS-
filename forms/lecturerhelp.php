...
$subject = trim($_POST['subject'] ?? 'Lecturer Account Assistance');
...
try {
    ...
    $mail->setFrom($mail->Username, 'CourseManager - Lecturer Help');
    if (!empty($email)) $mail->addReplyTo($email, $name);
    $mail->addAddress($recipient_email, 'Admin');

    $mail->isHTML(true);
    $mail->Subject = "Lecturer Help Request: {$subject}";
